...
/// - `path`: Optional jot path to display on second line
...
mod tests {
    ...
    fn test_visual_header_truncates_long_path() {
        let ctx = pretty_ctx();
        let long_path = "/a/very/long/path/that/exceeds/fifty/characters/should/be/truncated.jot";
        let h = header_with_context(&ctx, "list", None, Some(long_path));
        // Should contain ellipsis for truncated path
        assert!(h.contains("..."));
        assert!(h.contains("truncated.jot"));
    }
