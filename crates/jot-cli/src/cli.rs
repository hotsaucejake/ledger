...
/// Jot - A secure, encrypted, CLI-first personal journal and logbook
#[derive(Parser)]
#[command(name = "jot")]
#[command(author, version = VERSION, about, long_about = None)]
#[command(propagate_version = true)]
pub struct Cli {
    /// Path to the jot file
    #[arg(short = 'p', long, global = true, env = "JOT_PATH")]
    pub jot: Option<String>,
...
#[derive(Args)]
pub struct InitArgs {
    /// Path where the jot will be created
    #[arg(value_name = "PATH")]
...
#[derive(Subcommand)]
pub enum Commands {
    /// Initialize a new encrypted jot
    Init(InitArgs),

    /// Add a new entry to the jot
    Add(AddArgs),
