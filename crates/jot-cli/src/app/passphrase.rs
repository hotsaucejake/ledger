...
pub fn open_storage_with_retry(
    ...
    let env_passphrase = std::env::var("JOT_PASSPHRASE")
        .ok()
...
fn open_with_retry_prompt(
    ...
    let test_attempts = if !interactive && cfg!(feature = "test-support") {
        std::env::var("JOT_TEST_PASSPHRASE_ATTEMPTS")
            .ok()
            .map(|value| {
                value
                    .split(',')
...
fn is_incorrect_passphrase_error(err: &jot_core::error::JotError) -> bool {
