...
use jot_core::storage::{AgeSqliteStorage, NewEntryType, StorageEngine};
use uuid::Uuid;
...
pub fn handle_init(ctx: &AppContext, args: &InitArgs) -> anyhow::Result<()> {
    ...
    let passphrase = if let Ok(value) = std::env::var("JOT_PASSPHRASE") {
        if !value.trim().is_empty() {
            value
        } else if effective_no_input {
            return Err(anyhow::anyhow!(
                "--no-input requires JOT_PASSPHRASE for initialization"
            ));
        } else {
