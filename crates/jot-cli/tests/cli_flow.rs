...
fn temp_jot_path(prefix: &str) -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .expect("system time")
        .as_nanos();
    let filename = format!("{}_{}_{}.jot", prefix, std::process::id(), nanos);
...
fn open_sqlite_from_file(path: &PathBuf, passphrase: &str) -> Connection {
    ...
    let plaintext = jot_core::storage::encryption::decrypt(&encrypted, passphrase)
...
fn test_cli_init_writes_default_config() {
    ...
    assert!(keys.contains(&&"jot".to_string()));
...
fn test_cli_missing_config_message_uses_env_override() {
    ...
    let mut list = Command::new(bin());
    list.arg("list").env("JOT_CONFIG", &override_path);
...
fn test_cli_missing_jot_message() {
    ...
    list.arg("list")
        .arg("--jot")
        .arg(&missing)
        .env("JOT_PASSPHRASE", "test-passphrase-secure-123");
    apply_xdg_env(&mut list, &config_home, &data_home);
    let list = list.output().expect("run list");

    assert!(!list.status.success());
    let stderr = String::from_utf8_lossy(&list.stderr);
    assert!(stderr.contains("Jot file not found"));
...
fn test_cli_init_no_input_requires_passphrase() {
    ...
    assert!(stderr.contains("--no-input requires JOT_PASSPHRASE"));
}
...
fn test_cli_quickstart_output() {
    ...
    assert!(stdout.contains("jot init"));
}
...
fn test_cli_doctor_ok() {
    ...
    let mut doctor = Command::new(bin());
    doctor.arg("doctor").env("JOT_PASSPHRASE", passphrase);
    apply_xdg_env(&mut doctor, &config_home, &data_home);
    let doctor = doctor.output().expect("run doctor");
