...
/// * `salt` - Random salt (must be unique per ledger)
///
/// # Returns
///
...
/// - Different salt produces different key (salt must be stored with ledger)
/// - Memory-hard: requires ~64MB RAM, resistant to GPU attacks
///
/// # Examples
///
/// ```
/// use ledger_core::crypto::derive_key;
///
/// let salt = b"unique-salt-per-ledger-16bytes";
