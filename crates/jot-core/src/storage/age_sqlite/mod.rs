...
impl AgeSqliteStorage {
    ...
    fn lock_conn(&self) -> Result<MutexGuard<'_, Connection>> {
        self.conn
            .lock()
            .map_err(|_| JotError::Storage("SQLite connection poisoned".to_string()))
    }
    ...
    fn owned_data_from_bytes(bytes: &[u8]) -> Result<OwnedData> {
        if bytes.is_empty() {
            return Err(JotError::Storage("SQLite payload is empty".to_string()));
        }

        let size: i32 = bytes
            .len()
            .try_into()
            .map_err(|_| JotError::Storage("SQLite payload too large".to_string()))?;
        ...
        if raw.is_null() {
            return Err(JotError::Storage("SQLite allocation failed".to_string()));
        }
        ...
        unsafe {
            ...
            let ptr = NonNull::new(raw)
                .ok_or_else(|| JotError::Storage("SQLite allocation returned null".to_string()))?;
    ...
    fn write_atomic(path: &Path, data: &[u8]) -> Result<()> {
        let parent = path
            .parent()
            .ok_or_else(|| JotError::Storage("Invalid jot path".to_string()))?;

        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map_err(|e| JotError::Storage(format!("System time error: {}", e)))?
            .as_nanos();
        let filename = path
            .file_name()
            .and_then(|name| name.to_str())
            .ok_or_else(|| JotError::Storage("Invalid jot filename".to_string()))?;
        ...
        let mut file = OpenOptions::new()
            ...
            .create_new(true)
            .open(&temp_path)
            .map_err(|e| JotError::Storage(format!("Temp file create failed: {}", e)))?;
        use std::io::Write;
        file.write_all(data)
            .map_err(|e| JotError::Storage(format!("Temp file write failed: {}", e)))?;
        file.sync_all()
            .map_err(|e| JotError::Storage(format!("Temp file sync failed: {}", e)))?;

        crate::fs::rename_with_fallback(&temp_path, path)
            .map_err(|e| JotError::Storage(format!("Atomic rename failed: {}", e)))?;
...
impl StorageEngine for AgeSqliteStorage {
    fn create(path: &Path, passphrase: &str) -> Result<Uuid> {
        if path.exists() {
            return Err(JotError::Storage("Jot file already exists".to_string()));
        }
    ...
    fn open(path: &Path, passphrase: &str) -> Result<Self> {
        if !path.exists() {
            return Err(JotError::JotNotFound);
        }
        ...
        let device_id = Uuid::parse_str(&device_id_str)
            .map_err(|e| JotError::Storage(format!("Invalid device_id in metadata: {}", e)))?;
    ...
    fn metadata(&self) -> Result<JotMetadata> {
        ...
        let created_at = DateTime::parse_from_rfc3339(&created_at_str)
            .map_err(|e| JotError::Storage(format!("Invalid created_at timestamp: {}", e)))?
            .with_timezone(&Utc);
        ...
        let last_modified = DateTime::parse_from_rfc3339(&last_modified_str)
            .map_err(|e| JotError::Storage(format!("Invalid last_modified timestamp: {}", e)))?
            .with_timezone(&Utc);
    ...
    fn insert_entry(&mut self, entry: &NewEntry) -> Result<Uuid> {
        ...
        let schema_value: serde_json::Value = serde_json::from_str(&schema_json)
            .map_err(|e| JotError::Storage(format!("Invalid schema JSON: {}", e)))?;
        ...
        let tags_json = if normalized_tags.is_empty() {
            ...
            Some(
                serde_json::to_string(&normalized_tags)
                    .map_err(|e| JotError::Storage(format!("Failed to serialize tags: {}", e)))?,
            )
        };

        let data_json = serde_json::to_string(&entry.data)
            .map_err(|e| JotError::Storage(format!("Failed to serialize entry data: {}", e)))?;
    ...
    fn list_entries(&self, filter: &EntryFilter) -> Result<Vec<Entry>> {
        ...
        if let Some(ref tag) = filter.tag {
            ...
            let normalized_tag = normalized
                .first()
                .ok_or_else(|| JotError::Validation("Invalid tag filter".to_string()))?
                .clone();
            conditions.push(
    ...
    fn superseded_entry_ids(&self) -> Result<HashSet<Uuid>> {
        ...
        for row in rows {
            ...
            let parsed = Uuid::parse_str(&value)
                .map_err(|e| JotError::Storage(format!("Invalid supersedes UUID: {}", e)))?;
            ids.insert(parsed);
        }
        Ok(ids)
    }
        ...
        match result {
            Ok((id_str, name, version, created_at_str, device_id_str, schema_json_str)) => {
                let id = Uuid::parse_str(&id_str)
                    .map_err(|e| JotError::Storage(format!("Invalid UUID: {}", e)))?;
                let device_id = Uuid::parse_str(&device_id_str)
                    .map_err(|e| JotError::Storage(format!("Invalid device_id: {}", e)))?;
                let created_at = DateTime::parse_from_rfc3339(&created_at_str)
                    .map_err(|e| JotError::Storage(format!("Invalid timestamp: {}", e)))?
                    .with_timezone(&Utc);
                let schema_json: serde_json::Value = serde_json::from_str(&schema_json_str)
                    .map_err(|e| JotError::Storage(format!("Invalid JSON: {}", e)))?;
    ...
    fn create_entry_type(&mut self, entry_type: &NewEntryType) -> Result<Uuid> {
        ...
        let schema_json_str = serde_json::to_string(&entry_type.schema_json)
            .map_err(|e| JotError::Storage(format!("Failed to serialize schema: {}", e)))?;
    ...
    fn check_integrity(&self) -> Result<()> {
        ...
        if missing_fts > 0 {
            return Err(JotError::Storage("FTS index missing entries".to_string()));
        }
        ...
        if orphaned_fts > 0 {
            return Err(JotError::Storage("FTS index has orphaned rows".to_string()));
        }
    ...
    fn create_composition(&mut self, composition: &NewComposition) -> Result<Uuid> {
        ...
        let metadata_json = composition
            ...
            .transpose()
            .map_err(|e| JotError::Storage(format!("Failed to serialize metadata: {}", e)))?;
        ...
        match result {
            Ok((id, name, description, created_at, device_id, metadata_json)) => {
                ...
                let metadata = metadata_json
                    ...
                    .transpose()
                    .map_err(|e| JotError::Storage(format!("Invalid metadata JSON: {}", e)))?;
    ...
    fn rename_composition(&mut self, id: &Uuid, new_name: &str) -> Result<()> {
        ...
        if exists.is_none() {
            return Err(JotError::NotFound(format!("Composition {} not found", id)));
        }
    ...
    fn attach_entry_to_composition(
        ...
        if entry_exists.is_none() {
            return Err(JotError::NotFound(format!("Entry {} not found", entry_id)));
        }
        ...
        if comp_exists.is_none() {
            return Err(JotError::NotFound(format!(
    ...
    fn get_composition_entries(&self, composition_id: &Uuid) -> Result<Vec<EntryComposition>> {
        ...
        for row in rows {
            ...
            let entry_id = Uuid::parse_str(&entry_id)
                .map_err(|e| JotError::Storage(format!("Invalid entry UUID: {}", e)))?;
            let composition_id = Uuid::parse_str(&comp_id)
                .map_err(|e| JotError::Storage(format!("Invalid composition UUID: {}", e)))?;
    ...
    fn create_template(&mut self, template: &NewTemplate) -> Result<Uuid> {
        ...
        let template_json_str = serde_json::to_string(&template.template_json)
            .map_err(|e| JotError::Storage(format!("Failed to serialize template: {}", e)))?;
        ...
        match result {
            Ok((
                ...
                let entry_type_id = Uuid::parse_str(&entry_type_id)
                    .map_err(|e| JotError::Storage(format!("Invalid entry_type_id: {}", e)))?;
                let device_id = Uuid::parse_str(&device_id)
                    .map_err(|e| JotError::Storage(format!("Invalid device_id: {}", e)))?;
                let created_at = DateTime::parse_from_rfc3339(&created_at)
                    .map_err(|e| JotError::Storage(format!("Invalid timestamp: {}", e)))?
                ...
                let template_json: serde_json::Value = serde_json::from_str(&template_json)
                    .map_err(|e| JotError::Storage(format!("Invalid template JSON: {}", e)))?;
    ...
    fn update_template(&mut self, id: &Uuid, template_json: serde_json::Value) -> Result<i32> {
        ...
        let max_version =
            max_version.ok_or_else(|| JotError::NotFound(format!("Template {} not found", id)))?;
    ...
    fn delete_template(&mut self, id: &Uuid) -> Result<()> {
        ...
        if exists.is_none() {
            return Err(JotError::NotFound(format!("Template {} not found", id)));
        }
    ...
    fn set_default_template(&mut self, entry_type_id: &Uuid, template_id: &Uuid) -> Result<()> {
        ...
        let template_entry_type_id = template_entry_type_id
            .ok_or_else(|| JotError::NotFound(format!("Template {} not found", template_id)))?;
    ...
    fn get_default_template(&self, entry_type_id: &Uuid) -> Result<Option<Template>> {
        ...
        match result {
            ...
            Err(rusqlite::Error::QueryReturnedNoRows) => Ok(None),
            Err(e) => Err(JotError::Storage(format!("Database error: {}", e))),
        }
    }
}
