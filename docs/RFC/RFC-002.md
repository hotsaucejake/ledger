...
when the ledger file is synced or stored externally.
...
* If an entry type has a default template, `ledger add <type>` uses it automatically.
...
* **Template-driven quick add**: `ledger add gas --template last` reuses prior values.
