...
Ledger stores three kinds of things:

1. **Entry Types** — schema definitions (e.g., `journal`, `weight`)
2. **Entries** — instances of entry types (the actual data)
3. **Metadata** — global settings, FTS indexes, migration state
...
Each Ledger installation generates a stable device ID on first run:

```
~/.config/ledger/device_id
```
...
* This allows `ledger search 175` to find weight entries
* But structured queries (`--where "weight > 170"`) are preferred

### 9.3 FTS Synchronization
...
## 10. SQL Schema (For SQL Backends)
...
Initial values:
* `ledger_version` — app version that created this ledger
* `schema_version` — database schema version (for migrations)
* `created_at` — when ledger was created
* `device_id` — device that created this ledger

### 10.2 Entry Types Table
...
### 10.3 Entry Type Versions Table
...
* Ledger refuses to open newer schema versions
* Upgrade path is always forward

### 11.2 Entry Schema Migrations
...
* Optional `default_composition` field in entry type schemas
