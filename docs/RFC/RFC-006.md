...
**Phase:** 0.2 (design now, implement after core validation)
...
* When creating an entry of this type, Ledger auto-associates it with the default composition
...
This is stored as a name reference, resolved at entry creation time.
...
ledger compositions create project_x
ledger compositions create thesis_research --description "PhD work"
...
ledger compositions show project_x

# Rename composition
ledger compositions rename project_x project_alpha

# Delete composition (does not delete entries)
ledger compositions delete old_project --confirm
```
...
ledger add bookmark --url https://example.com --compose project_x

# Add entry to multiple compositions
ledger add journal --compose thesis_research --compose health_journey

# Override default composition
ledger add project_x_notes --compose different_project

# Skip default composition
ledger add project_x_notes --no-compose
```
...
ledger attach <entry-id> project_x

# Attach multiple entries
ledger attach <id1> <id2> <id3> project_x

# Detach entry from composition
ledger detach <entry-id> project_x
```
...
ledger list --compose project_x

# List specific type within composition
ledger list bookmark --compose project_x

# Search within composition
ledger search "vector clocks" --compose thesis_research

# Export composition
ledger export --compose project_x --json
```
...
ledger list --compose project_x
ledger search "important" --compose thesis_research

# Combine with other filters
ledger list todo --compose project_x --tag urgent --last 30d

# Future: query expressions
ledger list --where "compose = project_x AND completed = false"
```
...
If an entry type's `default_composition` references a deleted composition:

* Warning on entry creation
* Entry created without composition
* `ledger check` reports the orphan
...
| `ledger attach/detach` | No | Yes | Yes |
...
* Compositions live **inside the encrypted ledger**
* Composition names are unique (within ledger)
...
* Differentiates Ledger from simpler tools
