...
* **One logical jot** — users think of their data as a single thing
...
Jot defines a **Storage Engine interface** rather than hardcoding a single backend.
...
**Decision:** Jot Phase 0.1 uses **Age-encrypted SQLite** as its storage backend.
...
A Jot file (`.jot`) is an Age-encrypted SQLite database:
...
Jot supports two session modes:
...
```bash
jot open my.jot  # Opens, stays open
jot add journal     # Uses open session
jot close           # Explicitly closes
```
...
**Our position:** These are OS-level concerns. Jot's threat model targets file theft, not live system forensics. Users with higher security requirements should use full-disk encryption.
...
* Parameters stored in jot metadata
...
* Random 16+ bytes per jot
* Stored in jot metadata (not secret)
...
```bash
jot open ~/journals/personal.jot
jot open ~/journals/work.jot
```
...
* `jot backup` — copies jot + optional verify
...
* What's the maximum jot size before in-memory SQLite becomes problematic?
