...
* Theft of the ledger file (GitHub repo, backup, laptop disk image)
...
A Ledger file (`.ledger`) is an Age-encrypted SQLite database:

```
my.ledger = age_encrypt(passphrase, sqlite_database_bytes)
```
...
When user runs `ledger open my.ledger` (or any command requiring data):
...
```bash
ledger add journal    # Opens, adds, closes
ledger list           # Opens, lists, closes
```

**Persistent (for interactive use, Phase 0.2+):**
```bash
ledger open my.ledger  # Opens, stays open
ledger add journal     # Uses open session
...
Provide a `ledger doctor` command to benchmark and recommend settings.
...
Regardless of backend, the user interacts with "a ledger" as one thing.

### What the user sees

```bash
ledger open ~/journals/personal.ledger
ledger open ~/journals/work.ledger
```
...
* **Single file:** `personal.ledger` (SQLCipher or age-encrypted SQLite)
* **Directory:** `personal.ledger/` (GPG + files)
* **Archive:** `personal.ledger.tar.age` (encrypted tarball)
...
* `ledger doctor` checks for common security misconfigurations
...
* `ledger check` runs integrity verification
...
* Provide `ledger export --since <rev>` for mergeable patch format (future)
...
* `ledger check` — runs integrity check
* `ledger backup` — copies ledger + optional verify
* `ledger vacuum` — compacts (where applicable)
* `ledger export` — JSONL export for external backups

### Recommendation

Encourage regular `ledger backup` and Git commits. Document recovery procedures.
