...
| Check | Description |
|-------|-------------|
| Config exists | Is `~/.config/ledger/config.toml` present? |
| Config valid | Does config parse correctly? |
| Ledger exists | Does the configured jot file exist? |
| Ledger readable | Can we decrypt and read the jot? |
...
Ledger Doctor
─────────────
[OK] Config file found at ~/.config/ledger/config.toml
[OK] Config file is valid TOML
[OK] Ledger file exists at ~/.local/share/ledger/ledger.jot
[OK] Ledger file permissions are secure (600)
[OK] Editor configured: vim

All checks passed. Ledger is ready to use.
```
...
[OK] Config file found at ~/.config/ledger/config.toml
[OK] Config file is valid TOML
[WARN] Ledger file permissions too open (644)
       Hint: Run `chmod 600 ~/.local/share/ledger/ledger.jot`
...
```bash
ledger --ledger ~/work.jot add journal
