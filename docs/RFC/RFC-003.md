...
- If an entry type has a default template, `ledger add <type>` applies it automatically.
...
```bash
ledger doctor     # run all diagnostics
```
...
| Ledger exists | Does the configured ledger file exist? |
| Ledger readable | Can we decrypt and read the ledger? |
...
[OK] Ledger file exists at ~/.local/share/ledger/ledger.ledger
...
[WARN] Ledger file permissions too open (644)
       Hint: Run `chmod 600 ~/.local/share/ledger/ledger.ledger`
