...
### 3.2 `ledger-cli` (Human Interface)
...
* Everything goes through `ledger-core`
...
## 5. Module Boundaries (Inside `ledger-core`)
...
**Deliverable:** `ledger init`, `ledger unlock` work.
...
* [ ] `ledger add journal` opens $EDITOR
* [ ] `ledger list` shows entries
* [ ] `ledger show <id>` displays entry
* [ ] Tags work
...
* [ ] `ledger search <query>` returns results
...
* [ ] `ledger types create <name>`
* [ ] Interactive field definition
* [ ] Schema validation
* [ ] `ledger add <type>` with prompts
* [ ] Typo detection
...
* [ ] `ledger compositions create/list/show`
* [ ] `--compose` flag on `ledger add`
* [ ] `ledger attach/detach` commands
...
**Goal:** Reusable defaults stored inside the ledger for cross-device use.

* [ ] `templates` table
* [ ] `template_versions` table
* [ ] `ledger templates create/list/show`
* [ ] `ledger templates update/delete`
* [ ] `--template` flag on `ledger add`
